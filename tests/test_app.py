...
def test_add_contact(client):
    data = {'name':'Jane Doe','phone':'9876543210','email':'jane@example.com','type':'Personal'}
...
def test_update_contact(client, sample_contact):
    with app.app_context():
        contact = db.session.get(Contact, sample_contact)
        data = {
            'name': 'John Smith',
            'phone': contact.phone,
            'email': contact.email,
    ...
    response = client.post(f'/update/{sample_contact}', data=data, follow_redirects=True)
    assert response.status_code == 200

    with app.app_context():
        updated_contact = db.session.get(Contact, sample_contact)
...
def test_create_contact_api(client):
    data = {'name':'API User','phone':'5555555555','email':'api@example.com','type':'Work'}
